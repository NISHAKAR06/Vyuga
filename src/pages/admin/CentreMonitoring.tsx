        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="border-b border-slate-200 dark:border-slate-800 bg-slate-50/75 dark:bg-slate-800/40 text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
              <tr>
                <th className="px-5 py-3.5">Centre & Code</th>
                <th className="px-5 py-3.5">Farmers Today</th>
                <th className="px-5 py-3.5">Intake Tonnage</th>
                <th className="px-5 py-3.5">Avg Wait Time</th>
                <th className="px-5 py-3.5">Mandi Utilization</th>
                <th className="px-5 py-3.5 text-right">Load Status</th>
              </tr>
            </thead>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <span className="text-[10px] text-slate-400">{c.code} • {c.district}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4">
                    <span className="font-bold text-slate-900 dark:text-white">{c.totalFarmersToday}</span>
                    <span className="text-[10px] text-slate-400 block">Served: {c.servedToday} • Wait: {c.waitingNow}</span>
                  </td>
                  <td className="px-5 py-4 font-black text-emerald-600 dark:text-emerald-400">
                    {(c.capacityPerDay / 1000).toFixed(0)} Tons
                  </td>
                  <td className="px-5 py-4">
                    <span className="font-bold text-slate-900 dark:text-white">{c.currentAvgWaitMinutes} min</span>
                    <span className="text-[10px] text-purple-600 dark:text-purple-400 block">AI Pred: {c.predictedAvgWaitMinutes}m</span>
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      <span className="font-black text-slate-900 dark:text-white">{c.utilizationRate}%</span>
                      <div className="h-2 w-20 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                        <div
                          className={`h-full rounded-full ${
                            c.utilizationRate > 85 ? 'bg-rose-500' : c.utilizationRate > 65 ? 'bg-amber-500' : 'bg-emerald-500'
                          }`}
